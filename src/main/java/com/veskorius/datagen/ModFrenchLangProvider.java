...
public class ModFrenchLangProvider extends LanguageProvider {
    ...
    protected void addTranslations() {
        ...
        add("codex.veskorius.world.conduit_stone.text",
            "Le signe d'une ruine Architecte, comme la Pierre Veinée est le signe d'une poche de cristal. Elle est traversée de canaux — pas de veines. Une veine dit qu'il y a du cristal dedans ; un canal dit que quelqu'un a fait passer quelque chose ici.\n\nElle ne se fabrique pas, et une seule chose la mine : la Pioche en Alliage Veskorien. Ni le diamant ni la netherite n'en tirent quoi que ce soit.\n\nVous croiserez ces murs au palier 2, sans rien pouvoir en faire, et vous passerez votre chemin. Revenez avec l'alliage et ils donnent. Une ruine qui garde encore quelque chose quand on la retrouve vaut mieux qu'une ruine vidée d'un coup — et rien n'en dépend, donc attendre ne coûte aucune progression.\n\nSuivez un conduit allumé jusqu'à son terme et vous trouverez cette pierre : c'est par là que la ligne sort du mur.");
...
add("advancements.veskorius.tier2_field.title", "Réseau court");
        ...
        add("codex.veskorius.fields.harmonics.title", "Bandes harmoniques");
        add("codex.veskorius.fields.harmonics.text",
            "Tout champ porte l'une des trois bandes, et à partir du palier 3 toute machine en porte une aussi. Les deux se règlent à l'Accordeur de Résonance.\n\nUne machine sur la mauvaise bande FONCTIONNE QUAND MÊME. C'est tout le principe : rien ici ne refuse jamais de marcher. Elle coûte une fois et demie plus d'Osc, et elle injecte de la dissonance dans le champ — un point par tick de fonctionnement.\n\nUn montage désaccordé n'est donc pas cassé. Il est cher, et il empoisonne lentement le réseau dont il se nourrit. Vous ne le remarquerez pas tout de suite, et c'est exactement pour ça que ça vaut d'être su maintenant.\n\nLes machines du palier 1 n'ont aucune bande et acceptent n'importe quel champ : la boucle de départ ne gagne pas cette complexité.");
        add("codex.veskorius.fields.dissonance.title", "La dissonance, et ce qu'elle vous fait");
        add("codex.veskorius.fields.dissonance.text",
            "La dissonance est ce qu'une machine désaccordée laisse dans un champ. Elle s'accumule jusqu'à 2000, et elle s'annonce en trois étapes — vous êtes censé la voir à la première.\n\nUN : la coupole de particules grisaille. Rien ne va encore mal. DEUX : passé les trois quarts, le champ devient intermittent — les machines bégaient sans raison visible. TROIS : elle se décharge. Six dégâts dans un rayon de six blocs, la moitié du stock dépensée, puis cinq secondes avant que ça puisse recommencer.\n\nElle décroît d'elle-même d'un point par seconde, ce qui est plus lent qu'une seule machine désaccordée n'en produit. Laissé seul, un mauvais montage ne fait qu'empirer.\n\nDeux choses la retirent : un Réseau d'Amortissement, ou un Relais de Résonance — un relais renvoie la dissonance vers l'amont, vers sa source, plutôt que de la transmettre.");
        add("codex.veskorius.machines.damping.title", "Réseau d'Amortissement");
        add("codex.veskorius.machines.damping.text",
            "Le remède à la dissonance. Il purge le champ dans un rayon de 16 blocs, un cycle toutes les cinq secondes, et il consomme un agent pour le faire.\n\nTrois agents, qui ne diffèrent que par ce qu'ils absorbent : Cristal Raffiné 500, Flux Concentré 2500, Fragment de Résonance Météorique 6000. Le premier est disponible dès le palier 2, donc le remède n'arrive jamais après le problème.\n\nIl se remplit de Boue de Résonance à mesure — la dissonance, cristallisée. Cette boue n'est pas un rebut à jeter : elle fertilise les cultures comme un os à moelle, et un Récupérateur en retransforme quatre en Poussière de Résonance. Ce qu'on retire d'un réseau malade nourrit un champ.");
        add("codex.veskorius.machines.tunable_emitter.title", "Émetteur de Champ Accordable");
        add("codex.veskorius.machines.tunable_emitter.text",
            "Le même émetteur, avec une bande qu'on choisit. Craft : un Émetteur de Champ et deux Cristaux Raffinés — une amélioration, pas une seconde machine.\n\nL'émetteur ordinaire du palier 2 est mono-bande et ne vous demande rien. Celui-ci introduit le choix, et ce choix ne commence à compter qu'une fois que vos machines portent leur propre bande.\n\nFaites tourner deux réseaux sur des bandes différentes et ils ne se gêneront pas. C'est là tout l'intérêt : un champ est un lieu, et deux lieux peuvent se superposer sans se disputer.");
        ...
        add("codex.veskorius.machines.forge.text",
            "Deux Cristaux Raffinés et deux lingots donnent un lingot d'alliage en 20 secondes, à 4 Osc par tick. Le métal que vous lui donnez décide de la branche : le FER donne l'alliage structurel, l'OR l'alliage conducteur — et seul le conducteur entre dans un Relais. Ils ne se remplacent pas.\n\nChaque cycle laisse une Scorie de Flux dans son slot, et un slot plein arrête la Forge.");
